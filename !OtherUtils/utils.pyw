...
def printNodeInfos(nod, displayUnit):
    
    out.addout("Node number: " + str(nod[0]))
    out.addout("Node Coords: " + str(unitConv(nod[1], displayUnit)) + "," + str(unitConv(nod[2], displayUnit)) + "," + str(unitConv(nod[3], displayUnit)))
...
def normalize (cIni, cEnd):
    ...
    #    maxAbs = maxValue
    #elif (abs(maxValue) < abs(minValue)):
    #    maxAbs = minValue
...
#    norm = [0,0,0] 
#    if (maxAbs != 0):
#        norm = [vec[0]/maxAbs, vec[1]/maxAbs, vec[2]/maxAbs]
#
#    return norm
...
def rotVec (vec, grausX, grausY, grausZ, precision = 5):
    ...
    #Matriz de Rotacao 3D
    #rotX = [[1, 0, 0], [0, math.cos(radX), -math.sin(radX)], [0, math.sin(radX), math.cos(radX)]]
    #rotY = [[math.cos(radY), 0, math.sin(radY)], [0, 1, 0], [-math.sin(radY), 0, math.cos(radY)]]
    #rotZ = [[math.cos(radZ), -math.sin(radZ), 0], [math.sin(radZ), math.cos(radZ), 0], [0, 0, 1]]
    ...
    #vec = [ round(rotX[0][0]*vec[0] + rotX[0][1]*vec[1] + rotX[0][2]*vec[2], precision), 
    #        round(rotX[1][0]*vec[0] + rotX[1][1]*vec[1] + rotX[1][2]*vec[2], precision), 
    #        round(rotX[2][0]*vec[0] + rotX[2][1]*vec[1] + rotX[2][2]*vec[2], precision)]
#
    #vec = [ round(rotY[0][0]*vec[0] + rotY[0][1]*vec[1] + rotY[0][2]*vec[2], precision), 
    #        round(rotY[1][0]*vec[0] + rotY[1][1]*vec[1] + rotY[1][2]*vec[2], precision), 
    #        round(rotY[2][0]*vec[0] + rotY[2][1]*vec[1] + rotY[2][2]*vec[2], precision)]
#
    #vec = [ round(rotZ[0][0]*vec[0] + rotZ[0][1]*vec[1] + rotZ[0][2]*vec[2], precision), 
    #        round(rotZ[1][0]*vec[0] + rotZ[1][1]*vec[1] + rotZ[1][2]*vec[2], precision), 
    #        round(rotZ[2][0]*vec[0] + rotZ[2][1]*vec[1] + rotZ[2][2]*vec[2], precision)]
...
if (__name__ == "__main__"):
    ...
    #xL = normalize([4.5,7.2,6.1], [4.5,4.8,8.1])
    #print (str(globalToLocal ([1,0,0], xL, [1,0,0])))
    #print("---")
    print ("[1,0,0] : " + str(globalToLocal ([1,0,0], [1,0,0], [1,0,0])))
    print ("[0,1,0] : " + str(globalToLocal ([1,0,0], [1,0,0], [0,1,0])))
    print ("[0,0,1] : " + str(globalToLocal ([1,0,0], [1,0,0], [0,0,1])))
    print ("---")
    print ("[0,-1,0] : " + str(globalToLocal ([1,0,0], [0,1,0], [1,0,0])))
    print ("[1,0,0] : " + str(globalToLocal ([1,0,0], [0,1,0], [0,1,0])))
    print ("[0,0,1] : " + str(globalToLocal ([1,0,0], [0,1,0], [0,0,1])))
    print ("---")
    print ("[0,0,-1] : " + str(globalToLocal ([1,0,0], [0,0,1], [1,0,0])))
    print ("[0,1,0] : " + str(globalToLocal ([1,0,0], [0,0,1], [0,1,0])))
    print ("[1,0,0] : " + str(globalToLocal ([1,0,0], [0,0,1], [0,0,1])))
    print ("---")
    print ("[0.7,0,-0.7] : " + str(globalToLocal ([1,0,0], [1,0,1], [1,0,0])))
    print ("[0,1,0] : " + str(globalToLocal ([1,0,0], [1,0,1], [0,1,0])))
    print ("[0.7,0,0.7] : " + str(globalToLocal ([1,0,0], [1,0,1], [0,0,1])))
